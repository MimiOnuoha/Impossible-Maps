...
## Overview 
Even though the majority of the class will be taught in Python, we'll need Here's an overview (not exhaustive) of some additional, foundational things that we'll be using or referring to over the semester:
...
- **Markdown:** a lightweight text format that exports easily to HTML (as well as other formats) and allows us to write using any plain text editor.  
...
- **MacDown:** A client for writing Markdown. You can use Sublime Text for this as well, but using MacDown will make things much easier. 
...
5. **Change the _config.yml file**</br> We need to do a couple things in the configuration for the site. Click on the config.yml file, then click the pencil so you can edit the file. </br>Now delete or comment out (by placing a # in front of) any line that says `relative_permalinks: true` or `canonical URL`. </br> Change the Setup and the About/contact section so that they reflect your information. 
...
### Adding a new post
There are two ways to add a new post. The first is easier, the second way is harder but allows for more flexibility. Once you've added a new post, you'll want to go back and delete the old posts that came with the site, so that it is clear that it's yours. 
...
3. Make sure that you name your file in this style so that the most recent ones will always appear first:</br> YYYY-MM-DD-title-of-post </br> For example, you could title your post `2017-09-04-first-post`
